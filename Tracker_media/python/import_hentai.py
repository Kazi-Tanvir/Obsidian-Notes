...
# GraphQL Search Query specifically supporting genre filtering, isAdult, seasonYear, and pagination
SEARCH_QUERY = """
query ($page: Int, $search: String, $type: MediaType, $format: MediaFormat, $genreIn: [String], $isAdult: Boolean, $seasonYear: Int) {
  Page(page: $page, perPage: 10) {
    pageInfo {
      hasNextPage
    }
    media(search: $search, type: $type, format: $format, genre_in: $genreIn, isAdult: $isAdult, seasonYear: $seasonYear) {
      id
      title {
        romaji
        english
...
def search_anilist(query, media_type, format_filter=None, page=1, season_year=None):
    """Search AniList for adult media matching the Hentai genre with pagination and year filter."""
    variables = {
        "page": page,
        "type": media_type,
        "genreIn": ["Hentai"],
...
def show_interactive_search():
    """Menu interface to select media category, search, and import Hentai with pagination and year support."""
    print("\nSelect Hentai Media Category to Import:")
    print(" 1. Hentai Anime Series (OVA/ONA/TV)")
    print(" 2. Hentai Anime Movie")
    print(" 3. Hentai Manga / Doujinshi")
    print(" 0. Cancel")
    ...
    query = input("\nEnter title to search (leave blank to list popular titles): ").strip()
    
    year_input = input("Enter release year to filter (leave blank for any year): ").strip()
    season_year = None
    if year_input:
        if year_input.isdigit():
            season_year = int(year_input)
        else:
            print("Invalid year format. Searching without year filter.")
    ...
    while True:
        ...
        if season_year:
            desc_parts.append(f"released in {season_year}")
        desc_str = " " + " and ".join(desc_parts) if desc_parts else " popular"
        
        print(f"\nFetching{desc_str} Hentai from AniList (Page {current_page})...")
            
        data = search_anilist(query, media_type, format_filter, page=current_page, season_year=season_year)
        results = data.get("media", [])
        has_next = data.get("hasNextPage", False)
        
        if not results:
            print("No matching Hentai results found on AniList.")
            return

        print(f"\nSearch Results (Hentai Genre) - Page {current_page}:")
        ...
        for idx, r in enumerate(results, 1):
            ...
            print(f" {idx + start_num}. {name} ({year}) [Format: {fmt}] [ID: {r.get('id')}]")
            
        if has_next:
            print(" 99. Show more (Next page)")
        print(" 0. Cancel search")

        try:
            prompt_limit = start_num + len(results)
            sel = input(f"\nSelect an item to import ({start_num + 1}-{prompt_limit}), or 99 to show more, or 0 to cancel: ").strip()
            ...
            item_idx = val - start_num - 1
            if 0 <= item_idx < len(results):
                selected = results[item_idx]
                ...
                print(f"Selection out of range (Must be between {start_num + 1} and {prompt_limit}, or 99).")
        except Exception as e:
            print(f"Error during selection: {e}")

def main():
    print("=" * 80)
    print("                Importing Hentai Genre Media from AniList                     ")
    print("=" * 80)

    while True:
        print("\nMain Menu:")
        print(" 1. Search & Import Hentai Anime or Manga")
        print(" 0. Exit")
