...
- **STUN (Session Traversal Utilities for NAT)**: A lightweight server that reflects the client's public IP address and port mapping.

- **TURN (Traversal Using Relays around NAT)**: A relay fallback server used when direct P2P connection fails (e.g. both peers behind Symmetric NATs). Media packets are relayed through the TURN server.

- **ICE (Interactive Connectivity Establishment)**: The framework that gathers candidate endpoints (Host, Server Reflexive / STUN, Relayed / TURN) and tests connectivity pairs in parallel to find the lowest-latency viable path (**Trickle ICE**).
