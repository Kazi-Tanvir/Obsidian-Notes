...
- Calculate the minimum number of 64 KB Wasm pages required to allocate the full pixel buffer.

- Write a JavaScript helper that transfers the `ImageData` pixel array into Wasm linear memory, invokes an exported `invertPixels(ptr, length)` Wasm function, and writes the transformed pixels back to a canvas context without intermediate object allocations.
...
- Define a C-compatible struct format for 3D Game Entities:
...
- Implement `writeEntities(entities: Entity[]): { ptr: number; count: number }` that packs 10,000 entity records into a contiguous block in Wasm linear memory.

- Implement `readEntities(ptr: number, count: number): Entity[]` that reads back entity records by reference using `DataView` without cloning memory.
