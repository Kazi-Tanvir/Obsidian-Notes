...
- **Parsing (Lexing + Syntactic Analysis)**:
...
- Identifies and removes all `console.log()` and `console.debug()` `CallExpression` statements.

- Preserves `console.error()` and `console.warn()` statements.

- Completely cleans up parent `ExpressionStatement` nodes to prevent leaving empty dangling semicolons in generated code.
...
- Detects all `async function` declarations in the source code.

- Injects `const __start = performance.now();` at the very beginning of the function body.

- Wraps the existing function body in a `try...finally` block.

- Injects `console.log(`[Timer]: ${fnName} took ${performance.now() - __start}ms`);` into the `finally` block so that timing is logged on every execution even if the function throws an error.
