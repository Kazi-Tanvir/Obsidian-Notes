...
2.  Injects const __start = performance.now(); at the very beginning of the function body.

3.  Wraps the existing function body in a try...finally block.

4.  Injects console.log([Timer]: ${fnName} took ${performance.now() - __start}ms); into the finally block so that timing is logged on every execution even if the function throws an error.
