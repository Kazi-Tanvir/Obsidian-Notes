...
- **Determinism**: Given the same inputs, it always returns the exact same output.

- **No Side Effects**: It does not read or mutate external state (HTTP requests, global variables, DOM mutations, file I/O).
...
| --- | --- | --- |
| `pipe(...fns)` | `(...args) => fns.reduce((v, f) => f(v), args)` | Left-to-right function chaining |
| `compose(...fns)` | `(...args) => fns.reduceRight((v, f) => f(v), args)` | Right-to-left function chaining |
| `Array.prototype.toSorted()` | `arr.toSorted(compareFn)` | Non-mutating version of `sort()` |
| `Array.prototype.toSpliced()` | `arr.toSpliced(start, deleteCount, ...items)` | Non-mutating version of `splice()` |
...
- Allows partial application until `fn.length` arguments are supplied.

- Supports `curry._` as a placeholder to skip argument slots.
