...
- `Promise.all([promises])`: Fulfilled when **ALL** promises fulfill. Rejects **immediately** if any single promise rejects (short-circuit).

- `Promise.allSettled([promises])`: Waits for **ALL** promises to settle (either fulfill or reject). Never short-circuits. Returns an array of `{ status: "fulfilled", value }` or `{ status: "rejected", reason }`.

- `Promise.race([promises])`: Settles as soon as **ANY** single promise fulfills or rejects (fastest wins).

- `Promise.any([promises])`: Fulfilled as soon as **ANY** single promise fulfills. Rejects only if **ALL** promises reject (returns `AggregateError`).
...
- Limit max active concurrent promises to `concurrencyLimit` (e.g., max 3 at a time).

- Collect all errors without aborting valid items.
...
- Returns a Promise resolving to an array of objects: `{ status: 'fulfilled', value: ... }` or `{ status: 'rejected', reason: ... }`.

- Handles empty input arrays immediately.

- Preserves exact index ordering of the input array.
