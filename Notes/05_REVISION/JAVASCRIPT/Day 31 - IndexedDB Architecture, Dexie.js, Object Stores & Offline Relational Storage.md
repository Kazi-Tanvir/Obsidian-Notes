...
- Supports atomic `read(storeName, key)` and `write(storeName, value)`.

- Automatically handles transaction aborts, errors, and promise resolution on `tx.oncomplete`.

- Ensures transactions cleanly rollback if any operation within the batch throws.
