...
| --- | --- | --- |
| **Worker Instantiation** | `const w = new Worker('script.js')` | Creates background thread |
| **Memory Transfer** | `w.postMessage(data, [buffer])` | Transfers ownership without copying ($O(1)$) |
| **Service Worker Register** | `navigator.serviceWorker.register('/sw.js')` | Registers network proxy worker |
| **SW Fetch Intercept** | `self.addEventListener('fetch', fn)` | Intercepts HTTP requests for offline caching |
...
- Maintains a WebSocket connection with heartbeat ping/pong to detect stale connections.

- Implements an offline message queue that buffers outbound messages when offline (`readyState !== OPEN`) and flushes them sequentially upon reconnection.

- Uses exponential backoff with jitter for automatic reconnection.
