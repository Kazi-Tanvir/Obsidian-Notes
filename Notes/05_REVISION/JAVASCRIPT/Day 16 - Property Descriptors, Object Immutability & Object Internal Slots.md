...
- **Data Descriptor**: Contains a concrete value and mutation controls:
...
- **Accessor Descriptor**: Contains getter/setter functions instead of a static value:
...
- Recursively freezes all nested objects, arrays, and functions.

- Handles and prevents infinite recursion on **circular references** using a `WeakSet`.

- Preserves prototype chains and symbol properties (`Reflect.ownKeys`).
...
- Stores state internally with `configurable: false` and `enumerable: true`.

- Provides accessors (`get`, `set`) that automatically log every read and write attempt with timestamps and caller metadata.

- Completely disallows adding unauthorized properties by enforcing `Object.preventExtensions()`.

- Returns an immutable snapshot (`deepFreeze`) when calling `.getState()`.
