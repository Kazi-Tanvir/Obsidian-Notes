...
- **Default Binding**: Called standalone (`foo()`). Resolves to the global object (`window`/`globalThis`) in non-strict mode, or `undefined` in strict mode (`"use strict"`).

- **Implicit Binding**: Called via an owning object (`obj.foo()`). `this` binds to `obj`.

- **Explicit Binding**: Invoked with `call()`, `apply()`, or `bind()`. `this` is explicitly passed.
...
- `new` **Binding**: Called with `new Foo()`. A brand new object is constructed, and `this` binds to that new instance.
...
| --- | --- | --- |
| `fn.call(thisArg, ...args)` | Invokes function with explicit `this` | `fn.call(ctx, 1, 2)` |
| `fn.apply(thisArg, [args])` | Invokes function with array arguments | `fn.apply(ctx, [1, 2])` |
| `fn.bind(thisArg, ...args)` | Returns hard-bound function copy | `const bound = fn.bind(ctx)` |
...
- Ensure handler methods retain their class instance context without requiring callers to manually call `.bind()`.

- Use a `Proxy` wrapper to automatically auto-bind class methods upon instance creation.
...
- Triggers `onChangeCallback(path, newValue, oldValue)` whenever any property is modified.

- Supports deep nested objects (e.g. `store.user.profile.theme = "dark"` triggers callback with path `"user.profile.theme"`).

- Correctly handles array mutations (`push`, `pop`, `splice`).

- Uses `Reflect` methods for all internal operations.
