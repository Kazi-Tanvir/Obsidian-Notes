...
- Explain what value `sharedArray[0]` will contain if using standard assignment `sharedArray[0]++`.

- Explain why `Atomics.add(sharedArray, 0, 1)` guarantees exactly $400,000$ as the final counter.
...
- Use a designated 16-byte header: `[HeadIndex, TailIndex, Capacity, Flags]`.

- Implement `push(value: number): boolean` for Producer Worker (returns `false` if buffer full).

- Implement `pop(): number | null` for Consumer Worker (returns `null` if buffer empty).

- Enforce strict memory consistency using `Atomics.load` and `Atomics.store` without relying on mutex locks.
