...
- `Symbol.toPrimitive`: Overrides default `[ToPrimitive]` type conversion algorithm.

- `Symbol.hasInstance`: Customizes the behavior of the `instanceof` operator.

- `Symbol.isConcatSpreadable`: Dictates whether an object/array is flattened by `Array.prototype.concat`.

- `Symbol.species`: Controls the constructor function used to create derived objects in built-ins (e.g. `Array.prototype.map`, `Promise.prototype.then`).
...
| --- | --- | --- |
| `Symbol.toPrimitive` | `[Symbol.toPrimitive](hint: "number" \| "string" \| "default")` | Customizes type casting and arithmetic operations |
| `Symbol.hasInstance` | `static [Symbol.hasInstance](obj)` | Customizes `obj instanceof Class` evaluation |
| `Symbol.isConcatSpreadable` | `[Symbol.isConcatSpreadable]: boolean` | Configures array/object flattening in `.concat()` |
| `Symbol.species` | `static get [Symbol.species]() { return Constructor; }` | Overrides derived instance constructor in subclasses |
| `Symbol.toStringTag` | `get [Symbol.toStringTag]() { return "Tag"; }` | Customizes `Object.prototype.toString.call(obj)` tag |
...
- Provides a helper `attachHiddenMetadata(target: object, keyName: string, metadata: any): symbol` that attaches data under a unique symbol key.

- Ensures attached metadata is omitted from `JSON.stringify` and `Object.keys`.

- Implements `Symbol.for` namespacing to allow cross-module metadata retrieval via `getRegisteredMetadata(target: object, keyName: string)`.

- Overrides `[Symbol.toStringTag]` on the target object to display `[object SecureContainer]`.
