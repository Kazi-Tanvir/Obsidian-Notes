...
- **Capturing Phase**: Descends from `window` down to the target parent.

- **Target Phase**: Reaches the target element.

- **Bubbling Phase**: Ascends back up to `window`.
...
| `closest()` | `el.closest('.selector')` | Traverses up DOM tree to find matching ancestor |
...
- Listens for input changes across dynamically added input fields.

- Uses `MutationObserver` to automatically bind change listeners when new form input elements are dynamically injected.

- Debounces saves by 500ms before calling `onSaveCallback(formData)`.
