...
- **Marking**: Traces reachable objects starting from the GC Root (Global Object, Call Stack variables). V8 uses **Concurrent Marking** on background worker threads to avoid main-thread UI pauses.

- **Sweeping**: Scans memory ranges and adds unreferenced memory addresses back to free lists.

- **Compacting**: Relocates surviving objects into contiguous memory blocks to eliminate fragmentation.
...
- **Detached DOM Nodes**: Keeping JavaScript references to DOM elements that have been removed from the DOM tree.

- **Closure Scope Leaks**: Outer function variables retained indefinitely inside un-garbage-collected callbacks.

- **Uncleared Timers & Listeners**: Global `setInterval` or `EventEmitter` callbacks holding reference to enclosing scopes.
...
- The **Shallow Size** of `UserSessionManager`.

- The **Retained Size** of `UserSessionManager`.

- What happens to the memory when `manager = null` is executed?
...
- Pre-allocates an array of `N` instances using a factory function `allocator()`.

- Provides a `acquire()` method to retrieve an idle instance.

- Provides a `release(instance: T)` method to reset object state and return it to the pool.

- Prevents garbage collection churn during high-frequency short-lived object allocation loops (e.g. 100,000 requests/sec).
