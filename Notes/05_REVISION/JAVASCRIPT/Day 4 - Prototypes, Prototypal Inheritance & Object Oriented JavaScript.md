...
| `Object.getPrototypeOf(obj)` | Retrieves internal `[[Prototype]]` | `Object.getPrototypeOf(myDog) === Dog.prototype` |
| `Object.setPrototypeOf(obj, proto)` | Mutates internal `[[Prototype]]` (Slow - breaks V8 ICs) | `Object.setPrototypeOf(a, b)` |
...
| `super(...args)` | Invokes parent class constructor | Must be called before `this` in derived class |

### V8 Hidden Class Best Practices:

- Always initialize all object properties in the constructor.

- Never delete properties (`delete obj.prop`) — assign `null` or `undefined` instead.

- Keep property initialization order strictly uniform across instances.
...
- Private fields for sensitive properties (`#ssn`).

- Static builder method `Person.fromJSON(jsonString)`.
...
- Accepts `{ constructor, superclass, methods, staticMethods }`.

- Sets up proper prototype chain linking and constructor reference.

- Provides a working `this._super(methodName, ...args)` helper to invoke overridden parent methods.
