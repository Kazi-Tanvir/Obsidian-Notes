...
- `deriveKeyFromPassword(password: string, salt: Uint8Array)`: Uses `PBKDF2` with 600,000 iterations and SHA-256 to derive an AES-256-GCM encryption key.

- `encryptVaultItem(password: string, plainText: string)`:
...
- `decryptVaultItem(password: string, packedPayload: string)`:
