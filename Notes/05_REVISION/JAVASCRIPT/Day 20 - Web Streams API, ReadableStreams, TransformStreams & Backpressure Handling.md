...
- Accepts raw `string` chunks containing partial/fragmented JSON lines separated by `\n`.

- Buffers leftover text across chunk boundaries.

- Emits fully parsed JavaScript objects downstream as soon as a newline delimiter is encountered.
...
- Pipes an incoming binary `ReadableStream` through `CompressionStream("gzip")`.

- Intercepts chunks with a custom rate-limiting `TransformStream` that caps throughput to $5 \text{ MB/s}$ using `controller.desiredSize` and timestamp delays.

- Pipes the compressed output into a destination `WritableStream` that simulates slow network uploads without exceeding memory limits.
