...
- Supports chaining transformations (e.g. `map`, `filter`, `batch`).

- Streams item by item without buffering entire collections in memory.

- Handles thrown errors gracefully by calling `.return()` on upstream iterators to clean up resources.
