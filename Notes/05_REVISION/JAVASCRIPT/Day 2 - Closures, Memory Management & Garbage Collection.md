...
- **Old Generation (Old Pointer Space & Old Data Space)**:

- Stores long-lived objects that survived two Scavenger cycles.

- Managed using **Mark-Sweep-Compact Algorithm**.

- **Large Object Space**: Stores allocations exceeding single-page heap limits.

- **Code Space**: Stores JIT-compiled machine code instructions.
...
- **Accidental Global Variables**: Un-declared variables (`x = 10`) attach to `globalThis` / `window`.

- **Forgotten Timers / Callbacks**: `setInterval` keeping references to closed-over variables.

- **Detached DOM Nodes**: References to removed DOM nodes stored inside JS arrays/objects.

- **Uncleaned Event Listeners**: Event handlers holding references to large objects.
...
- Automatically uses `WeakRef` / `FinalizationRegistry` or strict internal reference cleanup so unreferenced values do not cause persistent leaks.

- Exposes `set(key, value)`, `get(key)`, `has(key)`, and `clear()`.
