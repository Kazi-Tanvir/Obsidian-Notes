...
- Uses `PerformanceObserver` to track LCP, CLS, and INP.

- Buffers metrics locally and sends them to `endpointUrl` using `navigator.sendBeacon()` when the page unloads/visibility changes to `hidden`.

- Ensures zero impact on main thread responsiveness and handles unobserve cleanup gracefully.
