...
- **Construction (Parsing & Fetching)**: Recursively parses import statements, fetches module files, and builds a **Module Record Graph**.

- **Instantiation (Linking)**: Allocates memory locations for all exported variables and links `import` identifiers in consuming modules to those memory locations (**Live Bindings**). No code is executed yet.

- **Evaluation**: Executes top-level code in post-order depth-first traversal and populates the linked memory locations with actual values.
...
| --- | --- | --- |
| **Named Export / Import** | `export const foo = 1;` <br> `import { foo } from './mod.js';` | Exports explicit identifiers |
| **Default Export** | `export default class App {}` <br> `import App from './mod.js';` | Exports single default payload |
...
| `__dirname` **in ESM** | `import { fileURLToPath } from 'url';` <br> `const __dirname = fileURLToPath(import.meta.url);` | Polyfills `__dirname` in ESM |
...
- Scans a target directory for `.plugin.js` or `.plugin.mjs` files.

- Dynamically imports each plugin using `import()`.

- Validates that each module exports a default object implementing `PluginInterface { name: string, version: string, init: () => Promise<void> }`.

- Initializes all valid plugins concurrently (`Promise.all`) and returns a Map of initialized plugin instances.
