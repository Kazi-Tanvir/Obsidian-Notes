...
- **Google V8 Engine**: Compiles JavaScript into native machine code, manages Call Stack & Memory Heap.

- **libuv (C Library)**: Multi-platform C library that handles the Event Loop, Thread Pool (`UV_THREADPOOL_SIZE`), non-blocking asynchronous I/O (epoll on Linux, kqueue on macOS, IOCP on Windows), and timers.

- **C++ Bindings & Core Modules**: Connects JS code (e.g., `fs`, `net`, `crypto`, `http`) with underlying C/C++ libuv primitives.
...
- **Pending Callbacks Phase**: Executes I/O callbacks deferred from the previous loop iteration (e.g. TCP errors).

- **Idle, Prepare Phase**: Used internally by libuv for system maintenance.
...
- Promise Microtask queue (`queueMicrotask` / resolved Promise `.then()`).
...
- Draw the architectural request flow showing API Gateway, Main Event Loop, Worker Thread Pool, and Redis Queue.

- Specify how the main Node.js Event Loop remains responsive under 10,000 concurrent API requests while offloading heavy hashing tasks.

- Define the error recovery, task timeout, and worker thread lifecycle management strategy.
...
- Implement `WorkerPool` class accepting `poolSize` and `workerScriptPath`.

- Maintain a queue of pending tasks when all workers are busy.

- Automatically replace crashed workers without dropping queued jobs.

- Expose `exec(taskData: any): Promise<any>` and `destroy(): Promise<void>`.

- Provide unit test scenarios simulating 20 concurrent tasks dispatched across 4 workers.
