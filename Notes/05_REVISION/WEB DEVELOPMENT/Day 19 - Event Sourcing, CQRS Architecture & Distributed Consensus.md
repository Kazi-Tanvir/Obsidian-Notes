...
- Detail the Event Store architecture ensuring zero double-spends and atomic balance transfers between accounts.

- Design the asynchronous Change Data Capture (CDC) projection pipeline (using Debezium + Kafka) that updates two distinct Read Models:

- Fast Customer Dashboard in Redis.

- Full-text Search & Audit History in Elasticsearch.

- Define the Snapshotting policy and Disaster Recovery strategy for hydrating cold aggregates.
...
- Implement the `OrderAggregate` with lifecycle commands:
...
- Build an `EventStore` class backed by an in-memory or PostgreSQL database with optimistic locking (`expectedVersion`) that rejects concurrent writes with `ConcurrencyConflictError`.

- Implement an asynchronous `OrderReadModelProjection` that processes events and maintains an updated view of active orders.

- Include comprehensive unit tests verifying event replay state hydration, optimistic concurrency conflicts, and invalid invariant rejections.
