...
- **Parsing**: The incoming query string is converted into an Abstract Syntax Tree (AST).

- **Validation**: The AST is validated against the statically typed Schema (SDL).

- **Execution**: The runtime traverses the AST recursively, invoking individual **Resolver Functions** at each field level.
...
- Detail how entities are shared and extended across subgraphs using `@key`, `@extends`, and `@external` federation directives.

- Diagram the Apollo Federation Router / Gateway layer that accepts unified client queries, compiles a query plan, and fans out sub-queries in parallel to backend subgraphs.

- Define the authorization strategy (JWT verification at the Gateway vs context header propagation to subgraphs).
...
- Implement a schema with `User`, `Organization`, and `Project` entities with full relation resolvers.

- Create dedicated `DataLoader` instances for `Organization.members` and `Project.contributors` to eliminate N+1 queries.

- Add a custom `depthLimit(5)` validation rule and a query cost analyzer middleware that rejects queries with costs $> 100$.

- Include mock test cases verifying that querying 50 projects with authors and organizations triggers exactly 3 total database batch calls instead of 101 calls.
