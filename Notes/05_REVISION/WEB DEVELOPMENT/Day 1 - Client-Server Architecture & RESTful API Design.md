...
- **Statelessness**: Every request contains all information needed to process it. Server stores no client session context.

- **Client-Server**: Separation of concerns between UI/UX and data storage/processing.

- **Cacheability**: Responses must define themselves as cacheable or non-cacheable (`Cache-Control`).

- **Uniform Interface**: Identification of resources via URIs (`/api/v1/users`), manipulation through representations, self-descriptive messages, HATEOAS.

- **Layered System**: Hierarchical layers (proxies, load balancers, gateways) transparent to the client.

- **Code on Demand (Optional)**: Executable code returned to client (e.g. JavaScript scripts).
...
- Define the full HTTP endpoint table (HTTP Verb, Path, Description, Request Headers, Status Codes).

- Write JSON Request and Response schemas for:
...
- Specify how tenant separation (`Tenant-ID`) and authentication token verification are handled architecturally.
...
- Implement a custom validation middleware that checks input payloads against a schema without letting invalid data reach the handler.

- Ensure every error is passed through a global error-handling middleware that returns standardized `{ success: false, error: string, timestamp: string }` response objects.

- Provide unit/integration test cases (specifying test inputs and expected status codes).
