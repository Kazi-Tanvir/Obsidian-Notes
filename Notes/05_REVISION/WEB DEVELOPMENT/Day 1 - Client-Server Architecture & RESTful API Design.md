...
2.  Write JSON Request and Response schemas for:
    ...
    a.  Adding an item to a cart.

    b.  Applying a promo code (handling edge cases like expired code or minimum purchase criteria).

    c.  Finalizing checkout (idempotent submission using Idempotency-Key header).
...
1.  Implement route handlers for:
    ...
    a.  POST /api/v1/users (Registration with field validation: email, password min 8 chars, role).

    b.  GET /api/v1/users (Supports query parameters for pagination: page, limit, and filtering: role).

    c.  PATCH /api/v1/users/:id (Partial update of profile information).
