...
| --- | --- | --- | --- | --- | --- |
| **LCP** | Largest Contentful Paint | $\le 2.5\text{s}$    $2. | \text{s} - 4.0\text{s}$     $> 4.0\ | ext{s}$    Slow server | TTFB, uncompressed hero images, render-blocking CSS |
| **INP** | Interaction to Next Paint | $\le 200\text{ms}$   $20 | \text{ms} - 500\text{ms}$   $> 500\ | ext{ms}$   Long JS exe | ution tasks (>50ms) blocking main thread on click |
