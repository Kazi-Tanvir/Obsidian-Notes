...
- **GIN (Generalized Inverted Index)**: Array types, JSONB fields, full-text search.

- **GiST / SP-GiST**: Geometric data, range types, nearest-neighbor searches.

- **BRIN (Block Range Index)**: Large append-only data (timeseries/logs) with minimal disk footprint.
...
- **Prisma Schema (**`schema.prisma`**)**: Declarative data model mapping models to SQL tables.

- **Prisma Client (TypeScript)**: Auto-generated type-safe client query builder.

- **Prisma Query Engine (Rust)**: High-performance binary executable that compiles Prisma query ASTs into optimized native SQL queries.
...
- Define the Prisma models for `Store`, `Product`, `Stock`, `Order`, and `OrderItem`.

- Implement **Optimistic Locking** using a `@default(1)` version column to prevent race conditions during high-concurrency item reservations.

- Write an architectural strategy to prevent deadlock scenarios when multiple buyers attempt to reserve the final stock items simultaneously.
...
- Implement a `transferFunds(fromAccountId: string, toAccountId: string, amount: number)` function inside a Prisma `$transaction`.

- Ensure accounts cannot go below a zero balance (throw operational error).

- Implement an exponential backoff retry wrapper (`retryTransaction(fn, maxRetries = 3)`) to automatically catch Postgres serialization failure/deadlock errors (SQLSTATE `40001` / `40P01`) and retry.

- Write test cases simulating 10 concurrent transfers between two accounts.
