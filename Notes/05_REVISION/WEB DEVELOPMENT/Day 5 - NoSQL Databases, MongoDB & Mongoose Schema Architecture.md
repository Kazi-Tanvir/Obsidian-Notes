...
- **Embedding (Denormalization)**: Nesting sub-documents inside a parent document.
...
- **Referencing (Normalization)**: Storing ObjectIds referencing documents in separate collections.
...
| --- | --- | --- |
| **Atomic Field Update** | `Model.updateOne({ _id }, { $inc: { views: 1 }, $set: { status: 'active' } })` | Avoids race conditions vs read-and-save |
| **Array Push** | `Model.updateOne({ _id }, { $push: { tags: 'node' } })` | Appends element to array field |
| **Populate Join** | `.populate('user', 'name email')` | Performs client-side `$lookup` join |
| **Aggregation Stage** | `[{ $match: { status: 'completed' } }, { $group: { _id: '$user', total: { $sum: '$totalAmount' } } }]` | High-performance analytics pipeline |
| **Text Search Index** | `schema.index({ title: 'text', description: 'text' })` | Enables `$text: { $search: 'keywords' }` |
...
- Demonstrate how naive 1-doc-per-reading creates indexing overhead and exceeds storage bounds.

- Apply the **Bucket Pattern** (grouping metrics by hour into a single document with pre-allocated arrays).

- Design the Mongoose schema and write an aggregation pipeline to calculate hourly average, max, and min temperature metrics.
...
- Create `UserWallet` and `TransactionHistory` schemas.

- Implement a function `transferBalance(senderId: string, receiverId: string, amount: number)` using MongoDB Session Transactions (`session.withTransaction`).

- Ensure atomic balance deduction (`$inc: { balance: -amount }`) with balance sufficiency checks (`balance: { $gte: amount }`).

- Provide unit tests simulating 5 concurrent transfer requests against a single wallet balance.
