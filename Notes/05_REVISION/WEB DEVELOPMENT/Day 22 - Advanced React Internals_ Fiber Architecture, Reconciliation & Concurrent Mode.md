...
- **Current Tree**: Reflects the nodes currently visible on the screen.

- **Work-In-Progress (WIP) Tree**: Built asynchronously in memory during rendering. Once complete, React swaps the root pointer (`current = workInProgress`) in a single $O(1)$ pointer assignment.
...
- Detail how to prevent main-thread UI freezing using Concurrent React (`useTransition`, `useDeferredValue`, and Web Workers).

- Design the component hierarchy separating volatile, fast-updating price ticks from slow-updating heavy SVG candlestick charts.

- Formulate an update batching and time-slicing strategy ensuring input fields (order submit forms) maintain sub-16ms latency under heavy render load.
...
- Implement a `FiberNode` structure with `child`, `sibling`, and `return` linked-list pointers.

- Implement a `workLoop` scheduler using `MessageChannel` (or `requestIdleCallback`) that processes a tree of 1,000 synthetic Fiber nodes in $5\text{ms}$ time-sliced chunks.

- Support **Priority Interruption**: If a high-priority task is queued while the scheduler is processing low-priority nodes, pause the current work-in-progress tree, execute the high-priority task, and resume or restart the low-priority tree.
