...
- **Request Memoization**: Deduplicates identical `fetch` requests within a single render pass.

- **Data Cache**: Persists fetched data across server requests (`fetch(url, { cache: 'force-cache' })`).

- **Full Route Cache**: Caches HTML and RSC payload at build time or revalidation window.

- **Router Cache**: In-memory client-side cache storing route segments.
...
- Demonstrate how sequential `await` calls create waterfall bottlenecks and how to resolve them with `Promise.all` and React `Suspense` streaming boundaries.

- Define `loading.tsx` and skeletal loading strategies for independent UI blocks.

- Architect an on-demand revalidation pipeline using Server Actions and `revalidateTag`.
...
- Server Component page that fetches product list from an API with tagged caching (`next: { tags: ['products-list'] }`).

- Client Component (`FilterBar.tsx`) that updates URL search parameters (`?category=electronics&sort=price_asc`) without full page refreshes using `useRouter` and `useSearchParams`.

- Server Component that reads `searchParams` prop to filter/sort database results.

- Provide a Server Action function `refreshProductCache()` that calls `revalidateTag('products-list')` to purge cached catalog data upon admin edits.
