...
- Diagram the inter-service communication flow across 3 services: `API Gateway` (REST/Next.js) -> `Order Matching Engine` (gRPC) -> `Risk Management Service` (gRPC).

- Formulate `.proto` schema definitions for `PlaceOrder`, `CancelOrder`, and `StreamLiveOrderBook` (Server Streaming).

- Detail how gRPC Deadline propagation and Envoy Load Balancing prevent cascade failures during sudden market volatility volume spikes (100k requests/sec).
...
- Write a `.proto` file defining `UserService` with:
...
- Implement the gRPC server handling requests with input validation, returning standard `grpc.status` codes (`NOT_FOUND`, `INVALID_ARGUMENT`).

- Build a client wrapper class `UserServiceClient` that configures a 2-second Deadline timeout, automated connection retry with exponential backoff, and a unary Promisified call wrapper.

- Include unit/integration tests verifying deadline timeout handling and successful protobuf serialization.
