...
- Define the folder structure featuring 3 apps (`apps/web`, `apps/admin`, `apps/api`) and 4 shared packages (`packages/ui`, `packages/db`, `packages/tsconfig`, `packages/eslint-config`).

- Write the complete `turbo.json` task dependency pipeline for `build`, `test`, `lint`, and `type-check`.

- Explain how to prevent the `@repo/db` package (Prisma schema) from rebuilding when only a CSS file in `@repo/ui` changes.
...
- Create `packages/db/package.json` with workspace protocol exports (`exports: { ".": "./src/index.ts" }`).

- Implement `packages/db/src/index.ts` exporting a singleton Prisma Client instance (`prisma`).

- Configure `apps/web/package.json` to depend on `"@repo/db": "workspace:*"` and import `prisma` directly inside a Next.js Server Component page (`apps/web/app/users/page.tsx`).

- Ensure `pnpm turbo run build` executes `prisma generate` in `@repo/db` before building `apps/web`.
