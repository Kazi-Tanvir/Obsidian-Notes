...
- **Progressive Enhancement**: HTML forms `<form action={serverAction}>` work even before client-side JavaScript has finished loading or if JavaScript is disabled.

- **Co-located Security**: Server Actions automatically sanitize parameters, restrict execution to POST methods, and check `Origin` headers against `allowedOrigins`.

- **Composable Data Mutation**: Directly call ORM operations (Prisma/Mongoose), trigger cache revalidations (`revalidatePath`, `revalidateTag`), and perform redirects (`redirect()`) within a single server cycle.
...
- Formulate the state machine across 3 sequential steps: Shipping Address -> Payment Method -> Order Review & Submission.

- Design an Idempotent Server Action mutation ensuring duplicate clicks on "Place Order" do not trigger double credit card charges or duplicate inventory reservations.

- Detail how `revalidateTag` purges inventory caches across Edge nodes while keeping user cart session data private and isolated.
...
- Implement the Client Wizard Component using:
