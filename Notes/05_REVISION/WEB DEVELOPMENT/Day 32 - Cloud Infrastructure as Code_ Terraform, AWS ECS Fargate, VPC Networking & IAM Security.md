...
- **ECS Task Execution Role (**`execution_role_arn`**)**: Used by the AWS ECS Agent to pull private Docker images from AWS ECR, configure CloudWatch logs, and decrypt environment secrets from **AWS Secrets Manager** / SSM Parameter Store.

- **ECS Task Role (**`task_role_arn`**)**: Used by your running application container code at runtime (e.g. Node.js code accessing AWS S3 buckets or DynamoDB tables via AWS SDK).
...
- **Security Module (**`modules/security-groups`**)**:
