...
| --- | --- | --- | --- |
| **Global Hook / Use** | `app.use(fn)` | `fastify.addHook('onRequest', fn)` | Attach correlation IDs and basic request timing |
| **Route Protection** | `app.get('/path', authMw, handler)` | `fastify.get('/path', { preHandler: [auth] }, handler)` | Run validation and auth before business controllers |
...
| **Security Headers** | `app.use(helmet())` | `fastify.register(import('@fastify/helmet'))` | Mandatory in production environments |
...
- Draw the middleware pipeline flow from incoming TCP connection to final response.
...
- Implement a `validateSchema(schema: ZodSchema)` middleware that validates `req.body`, `req.query`, and `req.params`.

- Implement an in-memory **Token Bucket Rate Limiter** middleware (`rateLimiter(maxRequests: number, windowMs: number)`) that attaches rate limit headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`).

- Wire everything into a sample endpoint `POST /api/v1/transfer` with custom operational error handling (`AppError`).

- Provide unit tests covering rate limit triggering (429 status) and schema validation failure (400 status).
